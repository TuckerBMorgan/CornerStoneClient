...
constexpr auto DEFAULT_PORT = "7878";
    ...
    do {

        {
            ...
            while (thead_parameters.to_server_message_channel->empty() == false) {
                ...
                auto a_result = send(ConnectSocket, send_buffer, jump.size(), 0);
            }
        ...
        if (iResult > 0) {
            auto a_test = find_network_messages(recvbuf, iResult);
            thead_parameters.message_channel_lock.lock();
            for (auto message : a_test) {
